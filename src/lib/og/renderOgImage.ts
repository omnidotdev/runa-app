...
let logoCache: { dataUri: string; width: number; height: number } | null = null;
...
const getLogo = (): { dataUri: string; width: number; height: number } => {
  if (logoCache) return logoCache;

  const logoBuffer = readFileSync(resolveLogoPath());
  // PNG intrinsic dimensions live in the IHDR chunk (width at byte 16, height at 20)
  logoCache = {
...
/**
 * Resolve an organization id from its workspace slug, or null when unknown.
 */
...
/**
 * Render a branded OpenGraph card (1200x630 PNG). Used for both project and
 * individual task share previews. Pass an optional `eyebrow` (e.g. a task key
 * and column) shown above the title, and an optional `accent` color.
 */
export const renderOgImage = async ({
  ...
  const logo = getLogo();
  // preserve the logo's aspect ratio at a fixed cap height so a non-square
  // wordmark is never squished into a square box
  const logoHeight = 36;
  ...
  const svg = await satori(
    {
      ...
      props: {
        ...
        children: [
            ...
            props: {
              ...
              children: [
                ...
                },
                {
                  type: "span",
                  props: {
                    style: { fontSize: "28px", color: "#ffffff", opacity: 0.9 },
                    children: "Omni",
                  },
                },
              ],
          ...
          // Middle: optional eyebrow + title + description
          {
            type: "div",
            props: {
              style: {
          ...
          // Bottom: Runa logo + name + workspace + domain
          {
            type: "div",
            props: {
              style: {
