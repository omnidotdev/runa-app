 ...
 * @see https://mswjs.io/docs/basics/response-resolver
 * @example
 * mockUsageMetricsQuery(
 *   ({ query, variables }) => {
 *     const { userId } = variables;
 *     return HttpResponse.json({
 *       data: { workspaces, projects, tasks }
 *     })
 *   },
 *   requestOptions
 * )
 */
export const mockUsageMetricsQuery = (resolver: GraphQLResponseResolver<Types.UsageMetricsQuery, Types.UsageMetricsQueryVariables>, options?: RequestHandlerOptions) =>
  graphql.query<Types.UsageMetricsQuery, Types.UsageMetricsQueryVariables>(
    'UsageMetrics',
    resolver,
    options
  )
