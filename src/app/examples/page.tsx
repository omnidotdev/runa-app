'use client';

import { CreateTaskForm } from '@/components/examples/CreateTaskForm';
import { TasksList } from '@/components/examples/TasksList';
import { Suspense } from 'react';

export default function ExamplesPage() {
  return (
    <div className="container mx-auto max-w-4xl p-6">
      <div className="mb-8">
        <h1 className="text-2xl font-bold">GraphQL and React Query Examples</h1>
        <p className="mt-2 text-gray-600">
          This page demonstrates how to use GraphQL and React Query in Runa App
        </p>
      </div>

      <div className="mb-8">
        <h2 className="mb-4 text-xl font-semibold">Create a New Task</h2>
        <div className="rounded-lg bg-white p-1 shadow dark:bg-gray-800">
          <CreateTaskForm />
        </div>
      </div>

      <div>
        <h2 className="mb-4 text-xl font-semibold">Tasks List</h2>
        <div className="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
          <Suspense fallback={<div>Loading...</div>}>
            <TasksList />
          </Suspense>
        </div>
      </div>
    </div>
  );
}